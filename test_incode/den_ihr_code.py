...
import task_generator_2 as tgen
...
def code_to_docstring(max_to_generate=128, temperature=0.4):
...
def docstring_to_code(max_to_generate=128, temperature=0.2):
    #task = task_generator.generate() #input()
...
#<|/ file |>'''
    ...
    return result["text"].replace("<|/ file |>","\n")

def code_completion(max_to_generate=1024, temperature=0.2):
    example = tgen.generate()
    result = generate(input=example,max_to_generate=max_to_generate,temperature=temperature)
...
if __name__ == "__main__":
    ...
    outputFolder = "output/" + str(datetime.now())+"generateMethodTest"
    os.mkdir(outputFolder)
    iter = 0
    while(True):
        print("docstring to code test:")
        #result = docstring_to_code(max_to_generate=1024,temperature=0.2) # standart ist 0.2
        result = code_completion(max_to_generate=1024, temperature=0.2)
        with open(outputFolder +"/"+ str(iter)+ ".py", 'w') as file:
            file.write(result)
        iter += 1